...
for k in sorted(seen): print(seen[k][0], k, '|', seen[k][1])")
    [ -n "$STATUSES" ] && ! echo "$STATUSES" | grep -q '^pending' && break
    sleep 5
  done
  if echo "$STATUSES" | grep -q '^failure'; then CI_STATE=failure; else CI_STATE=success; fi
  echo "  CI PR#$1: $CI_STATE"; echo "$STATUSES" | sed 's/^/     /'
}
has() { echo "$STATUSES" | grep -q "^$1 $2 "; }
...
curl -s $BASE/raw/branch/main/README.md | grep -q "contracts/orders-openapi.yaml" && ok "start README mentions the contract" || bad "start README"
...
git -C "$CL" diff --name-only main | grep -qx catalog-info.yaml && ok "branch adds catalog-info.yaml" || bad "branch content"
...
git -C "$CL" diff main -- .gitea/ | grep -q '^+  spectral-openapi-check:' && ok "diff shows the gate" || bad "diff"
...
git -C "$CL" diff main -- contracts/ | grep -q 'required: \[orderId, isPaid, currency\]' && ok "diff shows required currency" || bad "diff"
...
git -C "$CL" diff main -- .gitea/ | grep -q '^+  gateway-deploy-check:' && ok "diff shows the gate" || bad "diff"
...
git -C "$CL" diff main -- .gitea/ | grep -q '^+  breaking-changes-check:' && ok "diff shows the gate" || bad "diff"
...
git -C "$CL" diff main -- contracts/ | grep -q '^+.*name: channel' && ok "diff shows channel" || bad "diff"
...
job_log breaking-changes-check | grep -q 'new-required-request-parameter' && ok "oasdiff: new-required-request-parameter" || bad "oasdiff message"
OLD=$(head_sha $N)
